...
export default function InterviewSetup() {
  ...
  const selectedQuestionCount =
    QUESTION_COUNTS[config.duration] ?? 6
  ...
  const handleStart = async () => {
    ...
    const typeName =
      TYPES.find((item) => item.id === config.type)?.label ??
      config.type

    try {
      /*
       * Backend is the source of truth.
       * We create the interview first and use the real MongoDB ID.
       */
      ...
      /*
       * Keep Zustand state for other frontend pages.
       */
      startInterview({
        role: config.role,
        type: typeName,
      ...
      toast.success('Interview starting... Good luck! 🎯')

      /*
       * IMPORTANT:
       * Pass duration/config to InterviewRoom.
       *
       * This fixes the old issue where InterviewRoom started
       * with a default 10-minute timer because session was null.
       */
      ...
      /*
       * DO NOT create local-* interview here.
       *
       * The old local fallback created an ID that did not exist
       * in the backend, causing InterviewRoom API errors.
       */
    } finally {
      setLoading(false)
    }
    ...
    <div style={{ maxWidth: 1000 }}>
      ...
      >
        ...
        >
          ...
          >
            <Zap size={22} style={{ color: 'var(--blue-light)' }} />
          </div>
